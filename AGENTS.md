...
- No usar `localStorage`/`sessionStorage` directamente disperso por el código: centralizar en un módulo `lib/storage.js` (usado por el carrito persistente, entre otros). La única excepción es la cola de sincronización offline de la toma de pedidos internos (ver SRS RNF-12), que por volumen de datos usa IndexedDB en vez de localStorage — esa también se centraliza en un único módulo (`lib/offline-queue.js`), nunca disperso.
- **React Hook Form + Zod es obligatorio en todo formulario del sistema**, sin excepción: desde el login hasta el formulario más simple de un CRUD administrativo. No usar estado suelto de React (`useState` por campo) para manejar formularios donde ya existe este patrón estandarizado.
